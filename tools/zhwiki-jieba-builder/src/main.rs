...
#[derive(Parser, Debug)]
#[command(about = "Fast Chinese Wikipedia analyzer and game-index builder using jieba-rs")]
struct Args {
    #[arg()]
    ...
    #[arg(long, default_value = "../../data/zh/wikimedia")]
    input_dir: PathBuf,

    #[arg(long, default_value = "../../data/zh/wikimedia/stopwords")]
...
fn main() -> Result<()> {
    ...
    let count_progress = progress_bar(total_bytes, "Counting Chinese vocabulary")?;
    ...
    if let Some(output_dir) = &args.build_output_dir {
        let build_progress = progress_bar(total_bytes, "Building Chinese cooccurrence")?;
...
fn resolve_inputs(args: &Args) -> Result<Vec<PathBuf>> {
    if args.all_pages_articles_shards {
        let mut inputs: Vec<PathBuf> = std::fs::read_dir(&args.input_dir)
            ...
            .filter(|path| {
                path.file_name()
                    ...
                    .map(|name| {
                        name.starts_with("zhwiki-latest-pages-articles") && name.ends_with(".bz2")
                    })
                    .unwrap_or(false)
            })
            .collect();
        ...
        if inputs.is_empty() {
            bail!(
                "no zhwiki pages-articles shards found in {}",
    ...
    fn new() -> Result<Self> {
        Ok(Self {
            ...
            namespaced_link: Regex::new(
                r"\[\[\s*:?\s*(?:Category|File|Help|Image|Module|Portal|Special|Template|TimedText|Wikipedia|文件|档案|檔案|分类|分類|帮助|幫助|模板|维基百科|維基百科)\s*:[^\]]+\]\]",
            )?,
...
fn process_count_page(
    ...
    let title_tokens = tokenize_text(&page.title, jieba, stopwords, min_length);
    let clean_text = cleaner.clean(&page.text, &page.title);
    let text_tokens = tokenize_text(&clean_text, jieba, stopwords, min_length);
...
fn tokenize_text(
    ...
    jieba
        ...
        .filter(|token| is_han_only(token))
        .filter(|token| !stopwords.contains(*token))
        .map(ToOwned::to_owned)
        .collect()
}

fn is_han_only(s: &str) -> bool {
    !s.is_empty() && s.chars().all(|ch| matches!(ch, '\u{3400}'..='\u{4DBF}' | '\u{4E00}'..='\u{9FFF}' | '\u{F900}'..='\u{FAFF}'))
}
