...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-doc-ratio", type=float, default=0.4)
...
def load_stopwords(stopwords_dir: Path) -> set[str]:
    ...
    for file_path in sorted(stopwords_dir.glob("*.txt")):
...
def write_embedding_chunks(
    ...
    num_chunks = max(1, (num_words + words_per_chunk - 1) // words_per_chunk)
    words_per_chunk = max(1, (num_words + num_chunks - 1) // num_chunks)
...
def main() -> None:
    ...
    if weighted_matrix.shape[0] < 2 or weighted_matrix.shape[1] < 2:
        raise SystemExit("Not enough retained Ainu vocabulary to build embeddings.")
