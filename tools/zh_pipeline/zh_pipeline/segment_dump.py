...
import pynlpir
import regex
from pynlpir.cli import DATA_DIR as PYNLPIR_DATA_DIR, update_license_file

from .clean_wikicode import strip_wiki_markup
from .constants import DEFAULT_DATA_DIR, DEFAULT_OUTPUT_NAME, DEFAULT_XML_NAME
from .timing import iter_progress
from .xml_utils import iter_pages


HAN_WORD_PATTERN = regex.compile(r"\A\p{Han}+\Z")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Segment Chinese words from a Wikimedia XML dump shard."
    )
    parser.add_argument(
        "xml_path",
        nargs="?",
...
def init_segmenter() -> None:
    os.chdir(pynlpir.nlpir.PACKAGE_DIR)
    pynlpir.open(encoding="utf_8", encoding_errors="ignore")


def ensure_segmenter_license() -> None:
    try:
        os.chdir(pynlpir.nlpir.PACKAGE_DIR)
        pynlpir.open(encoding="utf_8", encoding_errors="ignore")
    except pynlpir.LicenseError:
        update_license_file(PYNLPIR_DATA_DIR)
        pynlpir.open(encoding="utf_8", encoding_errors="ignore")
    finally:
        try:
            pynlpir.close()
...
def segment_words(text: str) -> list[str]:
    return [
        token.strip()
        for token in pynlpir.segment(text, pos_tagging=False)
        if token.strip() and HAN_WORD_PATTERN.fullmatch(token.strip())
    ]
