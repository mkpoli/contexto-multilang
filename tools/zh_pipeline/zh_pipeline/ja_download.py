...
PAGES_ARTICLES_PATTERN = re.compile(
    r'href="(jawiki-latest-pages-articles\d+\.xml-p\d+p\d+\.bz2)"'
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Download a ja Wikipedia dump shard.")
    parser.add_argument("--url", default=DEFAULT_DUMP_URL, help="Dump URL to download.")
    parser.add_argument(
        "--all-pages-articles-shards",
        action="store_true",
        help="Download every jawiki pages-articles shard listed in the dump index.",
    )
    parser.add_argument(
        "--index-url",
