...
#[derive(Parser, Debug)]
#[command(about = "Fast Japanese Wikipedia lemma analyzer and game-index builder")]
struct Args {
    #[arg()]
    ...
    #[arg(long, default_value = "../../data/ja/wikimedia")]
    input_dir: PathBuf,

    #[arg(long, default_value = "../../data/ja/wikimedia/vibrato/system.dic.zst")]
    dictionary: PathBuf,
...
fn main() -> Result<()> {
    ...
    println!("Filtered lemma types: {}", selected.len());
...
fn load_dictionary_bytes(path: &Path) -> Result<Vec<u8>> {
    ...
    fn new() -> Result<Self> {
        Ok(Self {
            ...
            namespaced_link: Regex::new(
                r"\[\[\s*:?\s*(?:Category|File|Help|Image|Module|Portal|Special|Template|TimedText|Wikipedia|ファイル|カテゴリ|テンプレート|ウィキペディア)\s*:[^\]]+\]\]",
            )?,
...
fn process_count_page(
    ...
    for lemma in title_pairs
        .into_iter()
        .chain(text_pairs)
        .map(|(_, lemma)| lemma)
    {
        *counts.term_frequency.entry(lemma.clone()).or_insert(0) += 1;
        seen.insert(lemma);
    }
    for lemma in seen {
        *counts.document_frequency.entry(lemma).or_insert(0) += 1;
    }
...
fn tokenize_pairs(
    ...
    for i in 0..worker.num_tokens() {
        ...
        if let Some(lemma) = extract_lemma(surface, token.feature()) {
            if lemma.chars().count() >= min_length
                && !stopwords.contains(lemma.as_str())
                && !is_numeric_only(&lemma)
            {
                tokens.push((surface.trim().to_string(), lemma));
            }
        }
    }
    tokens
...
fn extract_lemma(surface: &str, feature: &str) -> Option<String> {
    let fields: Vec<&str> = feature.split(',').collect();
    let pos = fields.first().copied().unwrap_or("");
    if !matches!(pos, "名詞" | "動詞" | "形容詞" | "形状詞" | "副詞") {
        return None;
    }
    if pos.starts_with("記号") || pos.starts_with("補助記号") {
        return None;
    }
    for &index in &[10usize, 7, 8] {
        if let Some(candidate) = fields.get(index) {
            let candidate = candidate.trim();
            if !candidate.is_empty() && candidate != "*" && contains_wordlike(candidate) {
                return Some(candidate.to_string());
            }
        }
    }
    let trimmed = surface.trim();
    if contains_wordlike(trimmed) {
...
fn contains_wordlike(s: &str) -> bool {
    s.chars().any(|ch| {
        matches!(
            ch,
            '\u{4E00}'..='\u{9FFF}'
...
fn print_rank_samples(selected: &[SelectedToken], ranks: &[usize]) {
    ...
    for &rank in ranks {
        if rank == 0 || rank > selected.len() {
            println!("{rank:>6}: out of range (only {} lemmas)", selected.len());
            continue;
        }
...
fn write_tsv(path: &Path, selected: &[SelectedToken]) -> Result<()> {
    let mut output = String::from("rank\tlemma\tcount\tdoc_frequency\n");
...
fn build_state(
    ...
    for path in input_paths {
        ...
        walk_pages(path, progress.clone(), |page| {
            ...
            for (surface, lemma) in &title_pairs {
                if let Some(&token_id) = token_to_index.get(lemma.as_str()) {
                    add_alias(&mut state.variants, lemma, token_id);
                    add_alias(&mut state.variants, surface, token_id);
                }
            }

            let mut token_ids = Vec::new();
            for (surface, lemma) in &text_pairs {
                if let Some(&token_id) = token_to_index.get(lemma.as_str()) {
                    add_alias(&mut state.variants, lemma, token_id);
                    add_alias(&mut state.variants, surface, token_id);
...
fn write_game_artifacts(
    ...
    let old_to_new: HashMap<usize, usize> = kept_old_indices
        ...
        .collect();
    let variants = finalize_variants(build_state.variants, &old_to_new, selected);
